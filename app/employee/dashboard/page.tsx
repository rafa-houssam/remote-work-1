...
console.log(session); // Check if session.user.id exists


  
// useEffect(() => {
//   if (!userId) return; // wait until session loads
...
//   }, [userId]);
  ...
  if (!employee) return <p className="text-center mt-10">جارٍ التحميل...</p>;
    ...
    <EmployeeLayout>
      <div className="space-y-6" dir="rtl">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">مرحباً بعودتك، {employee.name}!</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <Card className="bg-gradient-to-r from-green-500 to-green-600 text-white">
            ...
            <CardContent>
              ...
              <p className="text-xs">{Math.round((employee.completedTasks / employee.totalTasks) * 100)}% إنجاز</p>
            </CardContent>
          </Card>
        ...
        <Card>
          ...
          <CardContent>
            {tasks.map(task => (
              <Card key={task._id} className="border-r-4 border-r-blue-500 mb-4">
                <CardHeader>
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="text-sm text-gray-600">
                        مُعيّنة من: {task.assignedBy?.name} - الاستحقاق: {task.dueDate?.split("T")[0]}
                      </div>
                    </div>
                ...
                <CardContent>
                  {task.status === "assigned" && (
                    <Button onClick={() => handleTaskStatusChange(task._id, "treated")}>
                      بدء العمل
                    </Button>
                  )}
                  {task.status === "treated" && (
                    <>
                      <Button onClick={() => handleTaskStatusChange(task._id, "done")}>تم الإنجاز</Button>
                      <Button variant="destructive" onClick={() => handleTaskStatusChange(task._id, "refused")}>رفض</Button>
                    </>
                  )}
                </CardContent>
              </Card>
